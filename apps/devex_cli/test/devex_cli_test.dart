...
void main() {
  test('version has human and stable machine representations', () async {
    final human = await DevExCli().run(<String>['version']);
    final machine = await DevExCli().run(<String>['--json', 'version']);

    expect(human.exitCode, 0);
    expect(human.stdout, 'devex 0.1.0-dev\n');
  ...
  test('unknown invocation uses the stable usage exit code', () async {
    final result = await DevExCli().run(<String>['unknown']);
  ...
  test('module bootstrap commands work without a workspace', () async {
    final empty = Directory.systemTemp.createTempSync('devex-modules-empty-');
    addTearDown(() => empty.deleteSync(recursive: true));
    final cli = DevExCli(workspaceDirectory: empty.path);
    ...
    expect(modules, hasLength(17));
    expect(
      modules.whereType<Map<String, Object?>>().where(
  ...
  test('module doctor reports the bounded AutoPreview claims', () async {
    final empty = Directory.systemTemp.createTempSync('devex-doctor-preview-');
    addTearDown(() => empty.deleteSync(recursive: true));

    final result = await DevExCli(workspaceDirectory: empty.path).run(<String>[
      '--json',
      'modules',
      'doctor',
      '--profile',
    ...
    expect(
      ...
      containsAll(<String>[
        ...
        'preview.detector.legacy-required',
  ...
  test('module explain reflects v2 profile without implying Android', () async {
    final workspace = Directory.systemTemp.createTempSync('devex-modules-v2-');
...
kit:
  ...
  modules: {}
''');
    final cli = DevExCli(workspaceDirectory: workspace.path);
...
''');
  ...
  test(
    'module doctor translates consumer config v1 to the legacy profile',
    () async {
      final workspace = Directory.systemTemp.createTempSync(
        'devex-modules-v1-',
      );
...
''');
      ...
      final result = await DevExCli(
        workspaceDirectory: workspace.path,
      ).run(<String>['--json', 'modules', 'doctor']);

      expect(result.exitCode, 0, reason: result.stderr);
      final payload = jsonDecode(result.stdout) as Map<String, Object?>;
      final value = payload['result']! as Map<String, Object?>;
      expect(value['profileId'], 'legacy-full-local-v1');
      expect(value['modules'], hasLength(17));
    },
  );

  test('config migrate previews and applies v1 to v2 recoverably', () async {
    final workspace = Directory.systemTemp.createTempSync('devex-config-cli-');
    addTearDown(() => workspace.deleteSync(recursive: true));
    Directory(p.join(workspace.path, '.devex')).createSync();
    final config = File(p.join(workspace.path, 'devex.yaml'))
      ..writeAsStringSync('''
schemaVersion: 1
content: {root: .devex}
workspace: {id: sample}
...
''');
    ...
    expect(File('${config.path}.v1.bak').readAsStringSync(), before);
  });

  test(
    'validate and explain are side-effect free while compile is atomic',
    () async {
      final workspace = Directory.systemTemp.createTempSync('devex-cli-');
      addTearDown(() => workspace.deleteSync(recursive: true));
      Directory(p.join(workspace.path, '.devex')).createSync();
      File(p.join(workspace.path, 'devex.yaml')).writeAsStringSync('''
schemaVersion: 1
...
''',
      ...
      final planPath = environment['DEVEX_RESOLVED_KIT_PLAN']! as String;
      final plan = ResolvedKitPlan.fromJson(
        jsonDecode(File(planPath).readAsStringSync()),
      );
      expect(environment['DEVEX_RESOLVED_KIT_PLAN_DIGEST'], plan.digest.value);
      expect(
  ...
  test(
    ...
    () async {
      final workspace = Directory.systemTemp.createTempSync('devex-cli-dev-');
      addTearDown(() => workspace.deleteSync(recursive: true));
      ...
      expect(
        Directory(
          p.join(workspace.path, '.dart_tool', 'devex', 'run'),
        ).existsSync(),
        isFalse,
      );
  ...
  test(
    ...
    () async {
      final workspace = Directory.systemTemp.createTempSync('devex-cli-hot-');
      addTearDown(() => workspace.deleteSync(recursive: true));
      _writeCatalog(workspace);
      final cli = DevExCli(workspaceDirectory: workspace.path);
      addTearDown(cli.closeDevelopmentRuns);
      ...
      expect(
        ...
        hostOrigin.replace(path: '/devex/bootstrap.json').toString(),
      );
      expect(await _httpStatus(hostOrigin.replace(path: '/health')), 200);
  ...
  test('doctor probes the real local toolchain', () async {
    final result = await DevExCli().run(<String>['--json', 'doctor']);
  ...
  test('capture and release build produce a verified local bundle', () async {
    final workspace = Directory.systemTemp.createTempSync('devex-cli-release-');
    addTearDown(() => workspace.deleteSync(recursive: true));
    _writeCatalog(workspace);
  ...
  test('session start negotiates and invokes a running Host', () async {
    ...
    final temporary = Directory.systemTemp.createTempSync('devex-cli-session-');
...
''');
    final application = DevExHostApplication(
    ...
    final result = await DevExCli().run(<String>[
      '--json',
      'session',
      'start',
      '--host',
  ...
  test('Gateway CLI drives the isolated Host sidecar lifecycle', () async {
    ...
    final workspace = Directory.systemTemp.createTempSync('devex-cli-gateway-');
    addTearDown(() => workspace.deleteSync(recursive: true));
    _writeCatalog(workspace);
...
''');
  ...
  test('Gateway doctor and sync use redacted local provider config', () async {
    final workspace = Directory.systemTemp.createTempSync('devex-cli-sync-');
    addTearDown(() => workspace.deleteSync(recursive: true));
    _writeCatalog(workspace);
...
''');
  ...
  test('init and detach are preview-first and ownership-aware', () async {
    ...
    expect(preview.exitCode, 0, reason: preview.stderr);
    expect(File(p.join(workspace.path, 'devex.yaml')).existsSync(), isFalse);
    expect(
      Directory(p.join(workspace.path, '.dart_tool')).existsSync(),
      isFalse,
    );
    ...
    expect(File(p.join(workspace.path, 'devex.yaml')).existsSync(), isTrue);
    expect(Digest.bytes(pubspec.readAsBytesSync()), pubspecDigest);
    expect(Digest.bytes(lock.readAsBytesSync()), lockDigest);

    final readme = File(p.join(workspace.path, '.devex', 'README.md'))
  ...
  test(
    ...
    () async {
      final result = await DevExCli(workspaceDirectory: _repositoryRoot())
          .run(<String>[
            '--json',
            'evidence',
            'collect-tests',
            '--target',
            'packages/devex_testkit/test/testkit_test.dart',
            '--timeout-seconds',
            '30',
          ]);
  ...
  test(
    'migration and retention commands preserve preview/apply semantics',
    () async {
      final workspace = Directory.systemTemp.createTempSync(
        'devex-cli-migration-',
      );
      addTearDown(() => workspace.deleteSync(recursive: true));
      final source = File(p.join(workspace.path, 'legacy.yaml'))
        ..writeAsStringSync('workspace: {id: imported, name: Imported}\n');
      final mapping = File(p.join(workspace.path, 'mapping.yaml'))
        ..writeAsStringSync('''
schemaVersion: 1
migrationId: cli-import
documents:
...
''');
      ...
      expect(
        File(
          p.join(workspace.path, '.devex', 'imported', 'workspace.json'),
        ).existsSync(),
        isTrue,
      );
      final retention = await cli.run(<String>[
        '--json',
        'retention',
        'status',
      ]);
      expect(retention.exitCode, 0, reason: retention.stderr);
      expect(
        (jsonDecode(retention.stdout) as Map<String, Object?>)['result'],
        containsPair('quotaBytes', 10 * 1024 * 1024 * 1024),
      );

      final rollback = await cli.run(<String>[
        '--json',
        'migrate',
        'rollback',
        '--apply',
  ...
  test(
    'V2 CLI composes source impact, context, bundle verification, and seal',
    () async {
      final workspace = Directory.systemTemp.createTempSync('devex-cli-v2-');
      addTearDown(() => workspace.deleteSync(recursive: true));
      _writeCatalog(workspace);
  ...
  test(
    ...
    () async {
      ...
      final linkText = File(
        p.join(workspace.path, '.devex', 'hosted-link.json'),
      ).readAsStringSync();
...
void _writeGatewayAuthoring(Directory workspace, List<int> fixtureBytes) {
  final root = Directory(p.join(workspace.path, '.devex', 'gateway'))
    ..createSync();
