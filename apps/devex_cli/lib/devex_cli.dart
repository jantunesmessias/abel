...
final class DevExCli {
  DevExCli({String? workspaceDirectory})
    : workspaceDirectory = workspaceDirectory ?? Directory.current.path;

  static const String version = '0.1.0-dev';

  final String workspaceDirectory;
  final Map<DevExDevelopmentSupervisor, List<StreamSubscription<ProcessSignal>>>
  _developmentRuns =
      <DevExDevelopmentSupervisor, List<StreamSubscription<ProcessSignal>>>{};
  ...
  Future<CliResult> run(List<String> arguments) async {
    ...
    if (command.name == 'config') {
      return _config(command, json: json);
    }
    ...
    if (command.name == 'migrate') {
      return _migration(command, json: json);
    }
  ...
  ArgParser _parser({Set<String>? enabledModuleIds}) {
    ...
    ArgParser gatewayLocal({required bool providerMandatory}) => ArgParser()
      ..addOption('config', abbr: 'c')
      ..addOption('local-config', defaultsTo: 'devex.local.yaml')
    ...
    ArgParser adoptionMutation() => ArgParser()
      ...
      ..addOption('distribution', defaultsTo: 'devex-kit')
    ...
    final distributionMigrate = distributionLocation()
      ..addFlag('dry-run', negatable: false)
      ..addFlag('apply', negatable: false);
    final distribution = ArgParser()
      ...
      ..addCommand('migrate-state', distributionMigrate)
      ..addCommand('rollback-migration', distributionLocation())
      ..addCommand(
        'verify-bundle',
    ...
    ArgParser applyMode() => ArgParser()
      ..addFlag('dry-run', negatable: false)
      ..addFlag('apply', negatable: false);
    final migration = ArgParser()
      ..addCommand(
        'legacy',
        applyMode()
          ..addOption('source', mandatory: true)
          ..addOption('mapping', mandatory: true)
          ..addOption('output', defaultsTo: '.devex/imported'),
      )
      ..addCommand(
        'verify',
        ArgParser()..addOption('output', defaultsTo: '.devex/imported'),
      )
      ..addCommand(
        'rollback',
        applyMode()..addOption('output', defaultsTo: '.devex/imported'),
      );
    ...
    final config = ArgParser()
      ..addCommand(
        'migrate',
        ArgParser()
          ..addOption('config', abbr: 'c')
          ..addOption('to', allowed: const <String>['2'], defaultsTo: '2')
          ..addFlag('dry-run', negatable: false)
          ..addFlag('apply', negatable: false),
      );
    final parser = ArgParser()
      ...
      ..addCommand(
        'adoption-report',
        ArgParser()..addOption('distribution', defaultsTo: 'devex-kit'),
      )
    ...
    if (enabled('automation.mcp')) {
      parser.addCommand('mcp', ArgParser()..addCommand('serve'));
    }
  ...
  CliResult _config(ArgResults command, {required bool json}) {
    final operation = command.command;
    final commandName = 'config ${operation?.name ?? ''}'.trim();
    try {
      if (operation?.name != 'migrate') {
        throw const FormatException('Usage: devex config migrate --to 2');
      }
      final dryRun = operation!.flag('dry-run');
      final apply = operation.flag('apply');
      if (dryRun == apply) {
        throw const FormatException(
          'Select exactly one of --dry-run or --apply',
        );
      }
      ...
      return _success(
        ...
        human: report.changed
            ? apply
                  ? 'Configuration migrated to v2; backup: ${report.backupPath}'
                  : 'Configuration can be migrated to v2 (dry-run)'
            : 'Configuration already uses v2',
        json: json,
  ...
  CliResult _modules(ArgResults command, {required bool json}) {
    ...
    try {
      if (operation == null ||
        ...
        throw const FormatException(
          'Usage: devex modules <list|explain|doctor>',
        );
      }
      final loaded = const CliKitPlanLoader().load(
        workspaceDirectory: workspaceDirectory,
  ...
  _autoPreviewRuntimeLimitations = <Map<String, Object?>>[
    ...
    <String, Object?>{
      'code': 'preview.detector.legacy-required',
      'message':
          'Flutter 3.44.8 LSP detection fails; --legacy-preview-detection is the verified workaround.',
    },
    <String, Object?>{
      'code': 'preview.export.runner-owned',
      'message':
          'Widget Previewer is interactive; the DevEx runner, not Previewer, exports PNG Evidence.',
    },
    <String, Object?>{
  ...
  Future<CliResult> _hostedAuth(
    ...
    try {
      if (operation == null) {
        throw const FormatException('Usage: devex auth <login|logout|status>');
      }
      ...
      callback.response
        ...
        ..write(
          error == null
              ? '<!doctype html><title>DevExKit</title>Authentication complete. You may close this window.'
              : '<!doctype html><title>DevExKit</title>Authentication failed.',
        );
  ...
  Future<CliResult> _hostedWorkspace(
    ...
    try {
      if (operation == null) {
        throw const FormatException('Usage: devex workspace <link|push|pull>');
      }
      final linkStore = _HostedLinkStore(
        p.join(workspaceDirectory, '.devex', 'hosted-link.json'),
      );
  ...
  CliResult _hostedResponse(
    ...
    final output = MachineOutput(
      ...
      failures: <DevExFailure>[
        DevExFailure(
          code: code is String ? code : 'DEVEX_HOSTED_FAILED',
          category: exit == 4
  ...
  String _credentialPath(String? explicit) {
    ...
    return p.join(configRoot, 'devex', 'hosted-auth.json');
  }
  ...
  Future<CliResult> _source(ArgResults command, {required bool json}) async {
    ...
    try {
      ...
      throw const FormatException(
        'Usage: devex source <inspect|diff> [options]',
      );
    } on FormatException catch (error) {
      return _failure(commandName, error.message, json: json);
    } on ArgumentError catch (error) {
  ...
  Future<CliResult> _context(ArgResults command, {required bool json}) async {
    ...
    try {
      if (operation?.name != 'export') {
        throw const FormatException('Usage: devex context export [options]');
      }
  ...
  Future<CliResult> _plugin(ArgResults command, {required bool json}) async {
    ...
    try {
      ...
      if (operation?.name != 'invoke') {
        throw const FormatException(
          'Usage: devex plugin <list|invoke> [options]',
        );
      }
  ...
  Future<CliResult> _mcp(ArgResults command, {required bool json}) async {
    if (command.command?.name != 'serve') {
      return _failure('mcp', 'Usage: devex mcp serve', json: json);
    }
  ...
  Future<CliResult> _releaseV2(ArgResults command, {required bool json}) async {
    ...
    try {
      switch (operation?.name) {
        case 'bundle':
          final built = const DeterministicDevExBundleRepository().build(
        ...
        default:
          throw const FormatException(
            'Usage: devex release <build|bundle|verify-bundle|seal> [options]',
          );
      }
    } on FormatException catch (error) {
      return _failure(commandName, error.message, json: json);
  ...
  Future<CliResult> _probe(ArgResults command, {required bool json}) async {
    ...
    if (operation?.name != 'run') {
      return _failure(
        'probe',
        'Usage: devex probe run --plan <json> --gateway-plan <json> --origin <loopback-origin>',
        json: json,
      );
    }
    ...
    try {
      ...
      if (!report.success) {
        final failure = DevExFailure(
          code: 'DEVEX_PROBE_FAILED',
  ...
  Future<CliResult> _migration(ArgResults command, {required bool json}) async {
    final operation = command.command;
    if (operation == null) {
      return _failure(
        'migrate',
        'Usage: devex migrate <legacy|verify|rollback>',
        json: json,
      );
    }
    final commandName = 'migrate ${operation.name}';
    try {
      final migrator = LegacyConfigurationMigrator(
        workspaceRoot: workspaceDirectory,
      );
      ...
      switch (operation.name) {
        case 'legacy':
          report = migrator.migrate(
            sourcePath: operation.option('source')!,
            mappingPath: operation.option('mapping')!,
            outputRoot: operation.option('output')!,
            apply: _mutationMode(operation),
          );
        case 'verify':
          report = migrator.verify(outputRoot: operation.option('output')!);
          if (!report.verified) {
            return _preconditionFailure(
              commandName,
        ...
        default:
          throw const FormatException('Unsupported migration operation');
      }
      return _success(
        command: commandName,
        result: report.toJson(),
        human: 'Legacy migration ${report.mode.name}: ${report.migrationId}',
        json: json,
        effectiveContext: <String, Object?>{'outputRoot': report.outputRoot},
      );
    } on FormatException catch (error) {
      return _failure(commandName, error.message, json: json);
    } on ArgumentError catch (error) {
  ...
  Future<CliResult> _retention(ArgResults command, {required bool json}) async {
    ...
    if (operation == null ||
      ...
      return _failure(
        'retention',
        'Usage: devex retention <status|gc>',
        json: json,
      );
    }
  ...
  Future<CliResult> _target(ArgResults command, {required bool json}) async {
    ...
    if (family?.name != 'android' || operation == null) {
      return _failure(
        'target',
        'Usage: devex target android <discover|bootstrap|update|remove|verify|install|launch|reset|capture>',
        json: json,
      );
    }
  ...
  Future<CliResult> _distribution(
    ...
    if (operation == null) {
      return _failure(
        'distribution',
        'Usage: devex distribution <install|status|rollback|migrate-state|rollback-migration|verify-bundle>',
        json: json,
      );
    }
    ...
    try {
      ...
      switch (operation.name) {
        ...
        case 'migrate-state':
          final apply = operation.flag('apply');
          if (apply && operation.flag('dry-run')) {
            throw const FormatException('Use either --apply or --dry-run');
          }
          result = (await manager.migrateState(apply: apply)).toJson();
        case 'rollback-migration':
          result = (await manager.rollbackMigration()).toJson();
        default:
  ...
  Future<CliResult> _evidence(ArgResults command, {required bool json}) async {
    ...
    if (operation == null ||
      ...
      return _failure(
        'evidence',
        'Usage: devex evidence <collect-previews|collect-tests|compare-visual|compare-semantics|export-artifact>',
        json: json,
      );
    }
  ...
  CliResult _adoption(ArgResults command, {required bool json}) {
    ...
    try {
      ...
      return _success(
        ...
        human: switch (name) {
          'init' when apply => 'DevExKit adoption applied',
          'init' => 'DevExKit adoption preview ready',
          'detach' when apply && !report.adopted => 'DevExKit detached',
          'detach' when apply => 'DevExKit partially detached',
          'detach' => 'DevExKit detach preview ready',
          _ =>
            report.adopted
                ? 'DevExKit adoption is active'
                : 'DevExKit adoption is not active',
        },
        json: json,
        effectiveContext: <String, Object?>{
  ...
  Future<CliResult> _runCatalogCommand(
    ...
    if (effectiveCommand == null ||
      ...
      throw const FormatException('Usage: devex release build [options]');
    }
    ...
    final migrated = loaded.documents
        .where((document) => document.wasMigrated)
        .toList(growable: false);
    ...
    final diagnostics = <MachineDiagnostic>[
      for (final document in migrated)
        MachineDiagnostic(
          code: 'DEVEX_ADJACENT_VERSION_MIGRATED',
          message: 'Read adjacent authoring version through the V0 migration.',
          context: <String, Object?>{'source': document.sourceName},
        ),
    ];
    switch (command.name) {
      case 'validate':
      ...
      case 'dev':
            ...
            result: <String, Object?>{
              ...
              'host': <String, Object?>{
                ...
                'environment': <String, String>{
                  'DEVEX_RESOLVED_KIT_PLAN': planPath,
                  'DEVEX_RESOLVED_KIT_PLAN_DIGEST':
        ...
        if (_developmentRuns.isNotEmpty) {
          throw StateError('This CLI instance already supervises devex dev');
        }
        ...
        return _success(
          ...
          human: runtime.studioOrigin == null
              ? 'DevEx Host ready at ${runtime.hostOrigin}'
              : 'DevEx Studio ready at ${runtime.studioOrigin}',
          json: json,
          effectiveContext: context,
          diagnostics: diagnostics,
        );
      case 'capture':
        ...
        if (input == null || input.isEmpty) {
          throw const FormatException('--input is required for V0 capture');
        }
          ...
          diagnostics: <MachineDiagnostic>[
            ...
            if (!fingerprint.hasReproductionClaim)
              const MachineDiagnostic(
                code: 'DEVEX_REPRODUCTION_CLAIM_DEGRADED',
                message:
  ...
  Future<CliResult> _doctor({required bool json}) async {
    ...
    return CliResult(
      exitCode: ok ? 0 : 3,
      stdout: ok ? 'DevExKit doctor: ready\n' : '',
      stderr: ok ? '' : 'DevExKit doctor: precondition failed\n',
    );
  }
  ...
  Future<CliResult> _session(ArgResults command, {required bool json}) async {
    ...
    if (start?.name != 'start') {
      return _failure(
        'session',
        'Usage: devex session start [options]',
        json: json,
      );
    }
    ...
    try {
      ...
      try {
        result = await connection.call('devex.session.start', <String, Object?>{
  ...
  Future<CliResult> _gateway(ArgResults command, {required bool json}) async {
    ...
    if (operation == null) {
      return _failure(
        'gateway',
        'Usage: devex gateway <run|status|apply-preset|verify|traffic|reset|stop|sync|doctor>',
        json: json,
      );
    }
    String requiredOption(String name) {
      ...
      try {
        ...
        switch (operation.name) {
          ...
          case 'apply-preset':
            final planDigest = _storeGatewayPlan(operation);
            result = await connection
                .call('devex.gateway.apply', <String, Object?>{
                  ..._gatewaySessionParams(operation),
                  'planArtifactDigest': planDigest.value,
                });
          case 'verify':
            ...
            result = await connection
                .call('devex.gateway.verify', <String, Object?>{
                  ..._gatewaySessionParams(operation),
          ...
          case 'traffic':
            result = await connection
                .call('devex.gateway.traffic', <String, Object?>{
                  ..._gatewaySessionParams(operation),
  ...
  CliResult _gatewayLocalPrecondition({
    ...
    final output = MachineOutput(
      ...
      failures: <DevExFailure>[
        DevExFailure(
          code: 'DEVEX_GATEWAY_PRECONDITION',
          category: FailureCategory.precondition,
          message: message,
          recoverability: Recoverability.userAction,
        ),
  ...
  Future<String?> _sourceRepositoryRoot() async {
    final library = await Isolate.resolvePackageUri(
      Uri.parse('package:devex_cli/devex_cli.dart'),
    );
  ...
  _gatewaySidecar(ResolvedKitPlan plan) async {
    ...
    final configured = Platform.environment['DEVEX_GATEWAY_COMMAND'];
    if (configured != null && configured.isNotEmpty) {
      ...
      if (decoded is! List<Object?> || decoded.any((item) => item is! String)) {
        throw const FormatException(
          'DEVEX_GATEWAY_ARGUMENTS_JSON must be a string array',
        );
      }
      return (
        command: configured,
    ...
    final packaged = File(
      p.join(p.dirname(Platform.resolvedExecutable), 'backend_gateway'),
    );
  ...
  void _watchDevelopment(DevExDevelopmentSupervisor supervisor) {
  ...
  Future<void> _stopDevelopment(DevExDevelopmentSupervisor supervisor) async {
  ...
  CliResult _preconditionFailure(
    ...
    final failure = DevExFailure(
      code: 'DEVEX_EVIDENCE_PRECONDITION',
      category: FailureCategory.precondition,
      message: message,
      recoverability: Recoverability.userAction,
    );
...
final class _HostedApiClient {
  ...
  Future<_HostedApiResponse> _send(
    ...
    request
      ...
      ..headers.set('x-devex-tenant', credential.tenantId)
...
final class _HostRpcConnection {
  ...
  static Future<_HostRpcConnection> connect({
    ...
    // The returned connection takes ownership and closes this socket.
    // ignore: close_sinks
    ...
    try {
      await connection.call('devex.initialize', <String, Object?>{
        'protocolVersion': 1,
        'sessionToken': token,
      });
      return connection;
