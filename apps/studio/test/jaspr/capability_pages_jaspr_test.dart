...
void main() {
  ...
  test('TargetFrame rejects a mixed Session and Scenario Lab run overlay', () {
    expect(
      () => TargetFrame(
        ...
        controller: TargetFrameController(),
        onAuthorizedMessage: (_) {},
      ),
      throwsArgumentError,
    );
  });
  ...
  testComponents('target route starts a Host session before mounting iframe', (
    ...
    await tester.click(find.componentWithText(StudioButton, 'Iniciar target'));
    await tester.pump();
    ...
    expect(
      find.byComponentPredicate(
        (component) =>
            component is TargetFrame && component.scenarioLabRunId == null,
        description: 'normal Session TargetFrame without Lab relay marker',
      ),
      findsOneComponent,
    );
    expect(find.textContaining('Target pronto'), findsOneComponent);

    await tester.click(find.componentWithText(StudioButton, 'Encerrar'));
    await tester.pump();
  ...
  testComponents(
    ...
    (tester) async {
      ...
      await tester.click(
        find.componentWithText(StudioButton, 'Iniciar Gateway'),
      );
      await tester.pump();
      await tester.pump();
...
WorkspaceSnapshot _snapshot() {
  ...
  return WorkspaceSnapshot(
    ...
    effectiveKitManifest: EffectiveKitManifest(
      ...
      rpcMethods: const <String>['session.start'],
