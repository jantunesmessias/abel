...
import 'target_frame_launch.dart';
...
final class TargetFrame extends StatefulComponent {
  ...
  /// The only observable projection of the private relay launch input.
...
final class _TargetFrameState extends State<TargetFrame> {
  late final String _frameId = 'workspace-target-${identityHashCode(this)}';
  ...
  void _activate() {
    ...
    scheduleMicrotask(() {
      ...
      final frameUri = buildTargetFrameLaunchUri(
        targetUri: component.targetUri,
        sessionId: component.sessionId,
        nonce: component.nonce,
        controllerOrigin: web.window.location.origin,
        gatewayOrigin: component.gatewayOrigin,
        gatewayDataOrigin: component._gatewayData?.value,
        scenarioLabRunId: component.scenarioLabRunId,
      );
      activateTargetFrameInOrder(
        ...
        navigate: () {
          final targetWindow = frame.contentWindow;
          if (targetWindow == null) {
            throw StateError('TargetFrame has no browsing context');
          }
          // Do not assign the launch location to `iframe.src`: the relay v2
          // Gateway origin is a Host-owned target input and must not become a
          // Studio DOM attribute. The initial about:blank context is
          // same-origin, so its Location can be replaced directly. The target
          // then reads the short-lived fragment through its runtime overlay.
          targetWindow.location.replace(frameUri.toString());
        },
      );
    });
  }
  ...
  void _onMessage(web.Event rawEvent) {
    ...
    try {
      final envelope = PostMessageEnvelope.decodeAndAuthorize(
        ...
        sourceMatches: identical(event.source, frame.contentWindow),
