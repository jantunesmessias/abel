  ...
  test(
    ...
    () async {
      ...
      final firstReady = await first
          .call('devex.gateway.initialize', <String, Object?>{
            'protocolVersion': 1,
      ...
      await first.call('devex.gateway.reset', const <String, Object?>{});
      expect(
        (await first.call(
          'devex.gateway.traffic',
          const <String, Object?>{},
...
''');
      ...
      expect(api.headers['x-devex-gateway']?.single, 'passthrough');
...
final class _Sidecar {
  ...
  static Future<_Sidecar> start(String repositoryRoot) async {
    final process = await Process.start(
      ...
      const <String>['run', 'apps/backend_gateway/bin/backend_gateway.dart'],
  ...
  Future<void> stop() async {
    if (stopped) return;
    await call('devex.gateway.stop', const <String, Object?>{});
