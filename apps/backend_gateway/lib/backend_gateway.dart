  ...
  Future<void> run(Stream<List<int>> input, IOSink output) async {
    ...
    await for (final line in lines) {
      ...
      try {
        ...
        if (request.method == 'devex.gateway.initialize') {
          if (application != null) {
          ...
          if (current == null) {
            throw StateError('devex.gateway.initialize is required first');
          }
          final result = switch (request.method) {
            'devex.gateway.status' => current.status(),
            'devex.gateway.apply' => current.applyPlan(
              CompiledGatewayPlan.fromJson(objectParams['plan']),
            ),
            'devex.gateway.verify' => await current.verify(objectParams),
            'devex.gateway.traffic' => current.traffic(objectParams),
            'devex.gateway.reset' => _reset(current),
            'devex.gateway.stop' => await _stop(current),
            _ => throw const _MethodNotFound(),
          };
          stop = request.method == 'devex.gateway.stop';
