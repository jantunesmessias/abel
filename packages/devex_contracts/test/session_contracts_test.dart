...
void main() {
  ...
  test('Session and Checkpoint are strict canonical public documents', () {
    ...
    expect(Checkpoint.fromJson(checkpoint.toJson()).digest, checkpoint.digest);
    expect(Session.fromJson(session.toJson()).digest, session.digest);

    final tampered = session.toJson()..['state'] = 'stopped';
    expect(() => Session.fromJson(tampered), throwsFormatException);
  });
