...
final class DevExBundleEntry {
  ...
  factory DevExBundleEntry.fromJson(Object? value) {
    final json = _bundleObject(value, 'DevExBundleEntry');
    ...
    return DevExBundleEntry(
      path: _bundleString(json, 'path', 'DevExBundleEntry'),
      digest: Digest(_bundleString(json, 'digest', 'DevExBundleEntry')),
      size: _bundleInt(json, 'size', 'DevExBundleEntry'),
      mediaType: _bundleString(json, 'mediaType', 'DevExBundleEntry'),
    );
  }
}

final class DevExBundleManifest {
  DevExBundleManifest({
    required this.releaseDigest,
    required this.releaseBundleDigest,
    required List<DevExBundleEntry> entries,
    this.attestationDigest,
  }) : entries = List<DevExBundleEntry>.unmodifiable(
         List<DevExBundleEntry>.of(entries)
    ...
    if (this.entries.isEmpty ||
        this.entries.map((entry) => entry.path).toSet().length !=
            this.entries.length) {
      throw ArgumentError(
        'DevExBundleManifest entries must be non-empty and unique',
      );
    }
  }
  ...
  final List<DevExBundleEntry> entries;
  ...
  factory DevExBundleManifest.fromJson(Object? value) {
    final json = _bundleObject(value, 'DevExBundleManifest');
    _bundleOnly(json, const <String>{
      'schemaVersion',
      'kind',
      'releaseDigest',
    ...
    if (json['schemaVersion'] != schemaVersion ||
        json['kind'] != 'DevExBundleManifest') {
      throw const FormatException(
        'Invalid DevExBundleManifest version or kind',
      );
    }
    final attestation = _bundleObject(
      json['attestation'],
      'DevExBundleManifest.attestation',
    );
    ...
    if (entriesValue is! List<Object?>) {
      throw const FormatException(
        'DevExBundleManifest.entries must be an array',
      );
    }
    final manifest = DevExBundleManifest(
      releaseDigest: Digest(
        _bundleString(json, 'releaseDigest', 'DevExBundleManifest'),
      ),
      releaseBundleDigest: Digest(
        _bundleString(json, 'releaseBundleDigest', 'DevExBundleManifest'),
      ),
      entries: entriesValue
          .map(DevExBundleEntry.fromJson)
    ...
    if (Digest(_bundleString(json, 'digest', 'DevExBundleManifest')) !=
        manifest.digest) {
      throw const FormatException(
        'DevExBundleManifest.digest does not match canonical content',
      );
    }
    return manifest;
  }
...
void _bundlePath(String value) {
  ...
  if (value.isEmpty ||
      ...
      segments.contains('') ||
      value.endsWith('/')) {
    throw FormatException('Unsafe DevExBundle path: $value');
  }
}
