...
final class CatalogCompiler {
  ...
  CatalogManifest compile(
    ...
    for (final journey in journeys) {
      ...
      for (final scenarioId in journey.scenarioIds) {
        if (!scenarioIds.contains(scenarioId)) {
          issues.add(
            'Journey ${journey.id} references unknown Scenario $scenarioId',
          );
        }
      }
    }
    for (final transition in transitions) {
      ...
      if (!scenarioIds.contains(transition.from) ||
          !scenarioIds.contains(transition.to)) {
        issues.add(
          'Transition ${transition.id} references an unknown Scenario',
        );
      }
    }
    ...
    return CatalogManifest(
      ...
      layout: layout ?? ConsumerLayout.devexDefault,
      workspace: workspace,
