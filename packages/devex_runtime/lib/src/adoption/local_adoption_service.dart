...
final class LocalAdoptionService {
  ...
  AdoptionManifest _desiredManifest({
    ...
    const roles = <String, String>{
      'devex.yaml': 'consumer-config',
      '.devex/README.md': 'living-documentation',
      '.devex/scenario.yaml': 'scenario-authoring',
      '.devex/journey.yaml': 'journey-authoring',
    };
  ...
  Map<String, List<int>> _desiredSources({
    ...
    return <String, List<int>>{
      'devex.yaml': utf8.encode('''schemaVersion: 1
content: {root: .devex}
workspace: {id: $workspaceId, displayName: $safeName}
applications:
  $applicationId: {root: ., target: local}
'''),
      '.devex/README.md': utf8.encode('''# Product journeys

This directory contains documentation authored for DevExKit. It does not alter
the production entrypoint, `pubspec.yaml`, or the consumer lockfile.
'''),
      '.devex/scenario.yaml': utf8.encode('''schemaVersion: 1
kind: Scenario
...
'''),
      '.devex/journey.yaml': utf8.encode('''schemaVersion: 1
kind: Journey
...
'''),
  ...
  void _atomicCreate(File target, List<int> bytes) {
    ...
    final temporary = File('${target.path}.devex-new-$pid');
  ...
  void _cleanupDetachedState() {
    ...
    if (entries.every((file) => file.path == lock.path)) {
      ...
      final devex = root.parent;
      if (devex.existsSync() && devex.listSync().isEmpty) devex.deleteSync();
      final dartTool = devex.parent;
