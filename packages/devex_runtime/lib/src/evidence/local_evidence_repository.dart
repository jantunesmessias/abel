...
final class LocalEvidenceRepository {
  ...
  Evidence? readLatestEvidence() {
    final pointerBytes = store.readStateBytes('evidence/latest.json');
    ...
    final evidenceBytes = store.readStateBytes(_evidencePath(digest));
    if (evidenceBytes == null) {
      throw StateError('EvidencePointer references a missing document');
    }
    final evidence = Evidence.fromJson(
      _decodeObject(evidenceBytes, _evidencePath(digest)),
    );
    if (evidence.digest != digest) {
      throw const FormatException('EvidencePointer digest mismatch');
    }
    return evidence;
  }
  ...
  void _writeDocument(String path, Map<String, Object?> document) {
    store.atomicWrite(path, _canonicalBytes(document));
  }
