...
final class DevExBundleBuildResult {
  const DevExBundleBuildResult({
    required this.path,
  ...
  final DevExBundleManifest manifest;
  final int size;
}

final class DeterministicDevExBundleRepository {
  const DeterministicDevExBundleRepository({
  ...
  DevExBundleBuildResult build({
    required String releaseDirectory,
    required String outputPath,
  }) {
    if (!outputPath.endsWith('.devexbundle')) {
      throw const FormatException('Bundle output must end with .devexbundle');
    }
    ...
    if (archive.length > maxTotalBytes + 8 * 1024 * 1024) {
      throw const FormatException('Encoded .devexbundle exceeds archive limit');
    }
  ...
  DevExBundleBuildResult verify(String bundlePath) {
    ...
    if (length <= 0 || length > maxTotalBytes + 8 * 1024 * 1024) {
      throw const FormatException('.devexbundle size is invalid');
    }
    ...
    final manifest = DevExBundleManifest.fromJson(manifestValue);
  ...
  DevExBundleManifest _manifest(Map<String, List<int>> entries) {
    final bundle = ReleaseBundle.fromJson(
      _decodeObject(entries['bundle.json'], 'bundle.json'),
    );
    final mediaTypes = <String, String>{
      'bundle.json': 'application/vnd.devex.release-bundle.v1+json',
      'release.json': 'application/vnd.devex.release.v1+json',
      'publication.json': 'application/vnd.devex.publication-view.v1+json',
      for (final artifact in bundle.artifacts)
        'blobs/sha256/${artifact.digest.value.substring('sha256:'.length)}':
            artifact.mediaType,
    };
    return DevExBundleManifest(
      releaseDigest: bundle.release.digest,
      releaseBundleDigest: bundle.digest,
      entries: <DevExBundleEntry>[
