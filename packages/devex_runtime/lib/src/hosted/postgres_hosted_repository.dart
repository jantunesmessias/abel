  ...
  Future<T> inTenantTransaction<T>(
    ...
    (session) async {
      await session.execute(
        Sql.named('''
          SELECT
            set_config('devex.tenant_id', @tenant:text, true),
            set_config('devex.principal_id', @principal:text, true)
        '''),
...
final class _PostgresHostedTransaction
  ...
  Future<Digest?> workspaceHead(String workspaceId) async {
    final rows = await _session.execute(
      Sql.named('''
        ...
        FROM devex_hosted.workspace_heads
  ...
  Future<void> insertRevision(WorkspaceRevision revision) async {
    ...
    await _session.execute(
      Sql.named('''
        INSERT INTO devex_hosted.workspace_revisions (
  ...
  Future<void> setWorkspaceHead(
    ...
    await _session.execute(
      Sql.named('''
        INSERT INTO devex_hosted.workspace_heads (
  ...
  Future<CollaborationEvent> appendEvent({
    ...
    final sequenceRows = await _session.execute(
      Sql.named('''
        ...
        FROM devex_hosted.collaboration_events
        WHERE tenant_id = @tenant:text AND workspace_id = @workspace:text
      '''),
      parameters: _workspaceParameters(workspaceId),
    );
    ...
    await _session.execute(
      Sql.named('''
        INSERT INTO devex_hosted.collaboration_events (
    ...
    await _session.execute(
      Sql.named('''
        INSERT INTO devex_hosted.outbox (
    ...
    await _session.execute("SELECT pg_notify('devex_collaboration', '')");
    return event;
  }
  ...
  Future<void> appendAudit(AuditEvent event) async {
    ...
    await _session.execute(
      Sql.named('''
        INSERT INTO devex_hosted.audit_events (
  ...
  Future<void> rememberPush({
    ...
    await _session.execute(
      Sql.named('''
        INSERT INTO devex_hosted.idempotency (
  ...
  Future<void> upsertPresence(PresenceLease lease) async {
    ...
    await _session.execute(
      Sql.named('''
        INSERT INTO devex_hosted.presence (
  ...
  Future<List<PresenceLease>> activePresence({
    ...
    await _session.execute(
      Sql.named('''
        DELETE FROM devex_hosted.presence
        WHERE tenant_id = @tenant:text
          AND workspace_id = @workspace:text
  ...
  Future<void> addComment(CommentThread thread) async {
    ...
    await _session.execute(
      Sql.named('''
        INSERT INTO devex_hosted.comments (
  ...
  Future<void> addApproval({
    ...
    required Approval approval,
      ...
      .execute(
        Sql.named('''
      INSERT INTO devex_hosted.approvals (
  ...
  Future<void> addRelease({
    ...
    for (final item in release.evidence) {
      await _session.execute(
        Sql.named('''
          INSERT INTO devex_hosted.evidence (
    ...
    await _session.execute(
      Sql.named('''
        INSERT INTO devex_hosted.releases (
