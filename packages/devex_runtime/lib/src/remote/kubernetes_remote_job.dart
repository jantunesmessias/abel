...
final class KubernetesRemoteJobConfiguration {
  KubernetesRemoteJobConfiguration({
    ...
    this.androidRuntimeClass = 'devex-android-kvm',
    ...
    this.androidAvdName = 'devex-api-35',
    ...
    this.sessionGatewayName = 'devex-public',
    this.sessionGatewayNamespace = 'devex-system',
...
final class KubernetesRemoteJobBuilder {
  ...
  KubernetesRemoteJobBundle build(RemoteAssignment assignment) {
    ...
    final suffix = namespace.substring('devex-run-'.length);
    final labels = <String, String>{
      'app.kubernetes.io/name': 'devex-remote-worker',
      'devex.dev/run': suffix,
      'devex.dev/tenant-hash': sha256
          ...
          'spec': <String, Object?>{
            ...
            if (isAndroid)
              'nodeSelector': <String, String>{
                'devex.dev/android-kvm': 'true',
                'devex.dev/android-image': configuration
                    .androidImageDigest
                    .value
                    .substring(7),
              },
            ...
            'containers': <Object?>[
              <String, Object?>{
                ...
                'args': <String>[
                  '--plan-file=/var/run/devex/plan.jwt',
                  '--capability-file=/var/run/devex/capability.jwt',
                  '--jwks-file=/etc/devex-trust/jwks.json',
                ],
                'env': <Object?>[
                  _env('DEVEX_WORKER_ID', assignment.lease.workerId),
                  _env('DEVEX_RUN_ID', plan.runId),
                  if (!isAndroid)
                    _env(
                      'DEVEX_CHROMIUM_PATH',
                      configuration.chromiumExecutable,
                    ),
                  if (isAndroid)
                    _env(
                  ...
                  if (isAndroid)
                    _env('DEVEX_ANDROID_AVD', configuration.androidAvdName),
                  if (isAndroid)
                    _env(
                      'DEVEX_SCRCPY_SERVER_PATH',
                      configuration.androidScrcpyServerPath,
                    ),
                  if (isAndroid)
                    _env(
                      'DEVEX_SCRCPY_SERVER_DIGEST',
                      configuration.androidScrcpyServerDigest.value,
                    ),
                  if (isAndroid)
                    _env(
                      'DEVEX_SCRCPY_VERSION',
                      configuration.androidScrcpyVersion,
                    ),
                  if (isAndroid)
                    _env(
                      'DEVEX_SCRCPY_PORT',
                  ...
                  if (isAndroid)
                    _env(
                      'DEVEX_GATEWAY_PORT',
                      configuration.gatewayPort.toString(),
                    ),
                  <String, Object?>{
                    'name': 'DEVEX_NAMESPACE',
                  ...
                  _env(
                    'DEVEX_CONTROL_ORIGIN',
                    configuration.controlPlaneOrigin.origin,
                  ),
                  _env(
                    'DEVEX_ARTIFACT_ORIGIN',
                    configuration.artifactOrigin.origin,
                  ),
                  _env(
                    'DEVEX_GATEWAY_ORIGIN',
                    configuration.gatewayOrigin.origin,
                  ),
                  if (plan.mode == RemoteRunMode.interactive)
                    _env(
                      'DEVEX_SESSION_GATEWAY_URL',
                      'ws://127.0.0.1:27183/v1/sessions/${plan.runId}/worker',
                    ),
                  if (!isAndroid && plan.mode == RemoteRunMode.interactive)
                    _env('DEVEX_WEB_INTERACTIVE_PORT', '27184'),
                ],
                'securityContext': const <String, Object?>{
              ...
              if (plan.mode == RemoteRunMode.interactive)
                <String, Object?>{
                  ...
                  'command': const <String>[
                    '/app/devex-remote-session-gateway',
                  ],
                  'ports': const <Object?>[
                    <String, Object?>{
                      'name': 'session',
                  ...
                  'env': <Object?>[
                    ...
                    _env('DEVEX_RUN_ID', plan.runId),
                    _env(
                      'DEVEX_SESSION_DEADLINE',
                      plan.expiresAt.toIso8601String(),
                    ),
                    _env(
                      'DEVEX_ALLOWED_STUDIO_ORIGINS',
                      configuration.allowedStudioOrigins.join(','),
                    ),
                    _env(
                      'DEVEX_REMOTE_JWKS_FILE',
                      '/etc/devex-trust/jwks.json',
                    ),
                    if (!isAndroid)
                      _env('DEVEX_WEB_TARGET_ORIGIN', 'http://127.0.0.1:27184'),
                  ],
                  'securityContext': const <String, Object?>{
                    'allowPrivilegeEscalation': false,
                    'capabilities': <String, Object?>{
                    ...
                    <String, Object?>{
                      'namespaceSelector': <String, Object?>{
                        'matchLabels': <String, String>{
                          'devex.dev/session-gateway': 'true',
                        },
                      },
                    },
                  ],
...
final class KubernetesApiRemoteJobLauncher implements RemoteJobLauncher {
  ...
  Future<void> cleanup(String namespace) async {
    if (!RegExp(r'^devex-run-[0-9a-f]{16}$').hasMatch(namespace)) {
