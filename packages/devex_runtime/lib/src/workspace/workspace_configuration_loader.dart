...
/// Filesystem boundary for both consumer config versions and the ignored local
/// overlay. No downstream module needs to reopen these files.
final class WorkspaceConfigurationLoader {
  ...
  LoadedWorkspaceConfiguration load({
    ...
    if (version != 1 && version != 2) {
      throw FormatException('$configPath: schemaVersion must equal 1 or 2');
    }
    final schemaVersion = version! as int;
    _validateMainDocument(document, schemaVersion, configPath);
    ...
    final contentRoot = _resolveInsideWorkspace(
      ...
      contentRootValue,
      path: r'$.content.root',
      requireStrictRelative: schemaVersion == 2,
      mustExist: true,
    );
    ...
    final launchProfiles = schemaVersion == 2
        ? _parseLaunchProfiles(
    ...
    final localKit = localDocument['schemaVersion'] == 2
        ...
        : const _ParsedKitLayer();
    final translatedProfile = schemaVersion == 1
        ? const LegacyV1ConfigurationTranslator().profileId
        : null;
  ...
  File _discoverConfig(Directory start) {
    var current = start;
    while (true) {
      final candidate = File(p.join(current.path, 'devex.yaml'));
      if (candidate.existsSync()) return candidate;
      final parent = current.parent;
      if (parent.path == current.path) break;
      current = parent;
    }
    return File(p.join(start.path, 'devex.yaml'));
  }
  ...
  void _validateMainDocument(
    ...
    if (version == 2 && !document.containsKey('kit')) {
    ...
    if (distribution != null) {
      ...
      _only(value, const <String>{'id', 'path'}, source, r'$.distribution');
      final id = _string(value, 'id', source, r'$.distribution');
      if (version == 2) OpaqueId.validate(id, 'Distribution');
      _optionalString(value, 'path', source, r'$.distribution');
    }
    ...
    if (version == 2) OpaqueId.validate(workspaceId, 'Workspace');
    ...
    for (final entry in applications.entries) {
      if (version == 2) OpaqueId.validate(entry.key, 'Application');
      final path = r'$.applications.' + entry.key;
    ...
    if (version == 2 && document['launchProfiles'] != null) {
...
final class LegacyV1ConfigurationTranslator {
  const LegacyV1ConfigurationTranslator();

  String get profileId => 'legacy-full-local-v1';
}
