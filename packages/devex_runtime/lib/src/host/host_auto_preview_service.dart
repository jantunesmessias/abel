...
final class HostAutoPreviewService {
  ...
  Future<bool> _renewArtifactHandles({
    ...
    _issuedHandles.removeWhere((handle) => handle.isExpiredAt(now));
    ...
    for (final projection in current.visualProjections) {
      ...
      projections.add(_withArtifactHandle(projection, handle));
    }
    ...
    if (changed) {
      await publishEvent('devex.workspace.changed', workspace.describe());
    }
    return changed;
  }
  ...
  List<VisualEvidenceProjection> _projectEvidence(
    ...
    for (final evidence in repository.readAllEvidence()) {
      if (evidence.policyId != 'auto-preview-v1' ||
  ...
  Future<void> _publishOperation(_HostPreviewOperation operation) =>
      publishEvent('devex.preview.changed', operation.toJson());
