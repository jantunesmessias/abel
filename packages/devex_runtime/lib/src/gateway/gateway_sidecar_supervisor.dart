...
final class GatewaySidecarSupervisor {
  ...
  Future<GatewaySidecarHandle> start({
    ...
    try {
      ...
      final dataOrigin = Uri.parse(_string(result, 'dataOrigin'));
      if (!dataOrigin.hasScheme ||
          InternetAddress.tryParse(dataOrigin.host)?.isLoopback != true) {
        throw const FormatException('Sidecar returned a non-loopback origin');
      }
