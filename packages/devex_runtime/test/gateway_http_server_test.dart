...
void main() {
  ...
  setUp(() async {
    temporary = Directory.systemTemp.createTempSync('devex-gateway-http-');
  ...
  test(
    ...
    () async {
      ...
      expect(api.headers['x-devex-gateway']?.single, 'mock');
      ...
      expect(report.response.headers['x-devex-gateway'], 'mock');
    },
  );
  ...
  test(
    ...
    () async {
      ...
      expect(unknown.headers['x-devex-gateway']?.single, 'denied');
