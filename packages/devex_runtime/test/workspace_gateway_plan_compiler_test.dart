...
void main() {
  ...
  setUp(() {
    workspace = Directory.systemTemp.createTempSync('devex-gateway-authoring-');
  ...
  test('compiles authoring without writes, then persists lazy CAS handles', () {
    ...
    final observedFixture = File(
      p.join(workspace.path, '.devex', 'fixtures', 'user.body'),
    ).readAsBytesSync();
  ...
  test('rejects missing presets and fixture traversal', () {
    ...
    final fixture = File(p.join(workspace.path, '.devex', 'fixture.yaml'));
...
void _writeWorkspace(Directory workspace) {
  final content = Directory(p.join(workspace.path, '.devex'))..createSync();
