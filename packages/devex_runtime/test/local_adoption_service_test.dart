...
void main() {
  ...
  setUp(() {
    workspace = Directory.systemTemp.createTempSync('devex-adoption-');
    File(
  ...
  test('dry-run is side-effect free and detach preserves modified files', () {
    ...
    final scenario = File(p.join(workspace.path, '.devex', 'scenario.yaml'))
  ...
  test('pre-existing files block apply without overwrite', () {
    ...
    expect(
      plan.files.singleWhere((file) => file.path == 'devex.yaml').state,
  ...
  test('symlinked content roots fail before writing outside workspace', () {
    final outside = Directory.systemTemp.createTempSync('devex-outside-');
    addTearDown(() => outside.deleteSync(recursive: true));
    Link(p.join(workspace.path, '.devex')).createSync(outside.path);
