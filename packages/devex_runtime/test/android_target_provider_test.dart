...
void main() {
  ...
  setUp(() {
    workspace = Directory.systemTemp.createTempSync('devex-android-workspace-');
  ...
  test('launch injects only the ephemeral resolved endpoint', () async {
    ...
    expect(launch, contains('DEVEX_GATEWAY_ORIGIN'));
    expect(launch, contains('http://127.0.0.1:51000'));
    expect(launch, contains('DEVEX_SCENARIO'));
  });
  ...
  test(
    ...
    () async {
      ...
      expect(
        runner.calls,
        contains(contains('shell pidof dev.devexkit.sample')),
      );
      expect(
        runner.calls,
  ...
  test(
    ...
    () async {
      ...
      await provider.resetPackage('emulator-5554', 'dev.devexkit.sample');
...
''');
    ...
    if (command.endsWith('shell dumpsys activity activities')) {
      return _text(
        'topResumedActivity=ActivityRecord{a1 u0 dev.devexkit.sample/.MainActivity t1}\n',
      );
    }
