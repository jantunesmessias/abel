...
void main() {
  final configuration = KubernetesRemoteJobConfiguration(
    webWorkerImage: 'registry.example.test/devex/worker-web@sha256:${'a' * 64}',
    androidWorkerImage:
        'registry.example.test/devex/worker-android@sha256:${'b' * 64}',
  ...
  test(
    ...
    () {
      ...
      expect(bundle.namespace, matches(RegExp(r'^devex-run-[0-9a-f]{16}$')));
  ...
  test('Android Job requires dedicated KVM resource without host mounts', () {
    ...
    expect(environment, contains('DEVEX_ANDROID_IMAGE_DIGEST'));
    expect(environment, contains('ANDROID_SDK_ROOT'));
    expect(environment, contains('DEVEX_ANDROID_AVD'));
  ...
  test(
    ...
    () async {
      final root = Directory.systemTemp.createTempSync('devex-kube-token-');
