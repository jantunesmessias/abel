  ...
  try {
    upstream.listen((request) async {
      if (request.uri.path == '/allowlisted-probe') {
        ...
        request.response.headers.set('x-devex-upstream', 'allowlisted');
      } else {
    ...
    target = await Process.start('unshare', <String>[
      ...
      'run',
      'packages/devex_runtime/tool/web_containment_target.dart',
    ]);
        ...
        try {
          ...
          target.stdin.writeln(
            jsonEncode(<String, Object?>{
              ...
              'allowlisted':
                  response.headers.value('x-devex-upstream') == 'allowlisted',
            }),
          );
        } finally {
