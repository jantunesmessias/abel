...
final class DevExRuntimeOverlay {
  DevExRuntimeOverlay._();

  static const MethodChannel _channel = MethodChannel(
    'devex.flutter/runtime-configuration-v1',
  );
  ...
  /// target's URL fragment by DevEx Studio. Fragments are not sent to the
  /// target HTTP server and the iframe uses a no-referrer policy.
  static Map<String, String> readWebLaunchOverlay({Uri? uri}) {
    if (!kIsWeb && uri == null) return const <String, String>{};
    final fragment = (uri ?? Uri.base).fragment;
    const prefix = 'devex-launch=';
  ...
  static Map<String, String> _validatedOverlay(
    ...
    for (final entry in values.entries) {
      if (!RegExp(r'^DEVEX_[A-Z0-9_]{1,58}$').hasMatch(entry.key) ||
