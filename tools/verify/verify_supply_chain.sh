...
if ! rg -q --fixed-strings \
  ...
  deploy/helm/control-plane/templates/configmap.yaml; then
...
for required in 'serviceAccountToken:' 'expirationSeconds: 600' \
  ...
  if ! rg -q --fixed-strings "$required" \
    deploy/helm/control-plane/templates/deployment.yaml; then
...
if ! rg -q --fixed-strings 'resources: ["namespaces"]' \
  deploy/helm/control-plane/templates/remote-rbac.yaml; then
  echo 'Helm remote runtime RBAC is missing.' >&2
  failed=1
fi

if rg -q --fixed-strings 'namespaceSelector: {}' \
  deploy/helm/control-plane/templates/network-policy.yaml; then
...
if ! rg -q 'image.digest is required; mutable tags are forbidden' \
  deploy/helm/control-plane/templates/_helpers.tpl; then
