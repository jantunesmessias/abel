...
standards_dir="$(mktemp -d /tmp/workspace-standards.XXXXXX)"

cleanup() {
  case "$standards_dir" in
    /tmp/workspace-standards.*) rm -rf -- "$standards_dir" ;;
...
dart run libs/experience_contracts/tool/standards_conformance.dart \
  "$standards_dir/json-schema" \
  "$standards_dir/jcs"
