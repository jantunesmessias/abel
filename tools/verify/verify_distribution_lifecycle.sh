...
temp_dir="$(mktemp -d /tmp/workspace-distribution-lifecycle.XXXXXX)"
cleanup() {
  local original_status=$?
  case "$temp_dir" in
    /tmp/workspace-distribution-lifecycle.*)
...
bundle_dir="$temp_dir/full-local-0.1.0-preview.1"
repro_bundle_dir="$temp_dir/full-local-0.1.0-preview.1-rebuild"
second_bundle_dir="$temp_dir/full-local-0.1.0-preview.2"
...
cd "$temp_dir"
"$bundle_dir/bin/workspace" --json distribution verify-bundle \
  --bundle "$bundle_dir"
"$repro_bundle_dir/bin/workspace" --json distribution verify-bundle \
  --bundle "$repro_bundle_dir" > "$temp_dir/repro-verify.json"
"$bundle_dir/bin/workspace" --json distribution install \
  --bundle "$bundle_dir" \
  --install-root "$install_dir"

"$install_dir/bin/workspace" --json version > "$temp_dir/canonical.json"
"$install_dir/bin/gateway_sidecar" --version

cd "$repo_dir"
dart run apps/workspace_cli/tool/repackage_distribution_rehearsal.dart \
  "$bundle_dir" "$second_bundle_dir" 0.1.0-preview.2
"$install_dir/bin/workspace" --json distribution install \
...
"$install_dir/bin/workspace" --json distribution rollback \
  --install-root "$install_dir" > "$temp_dir/rollback.json"
rg -q '"currentVersion":"0.1.0-preview.1"' "$temp_dir/rollback.json"

set +e
"$install_dir/bin/workspace_host" > /dev/null 2> "$temp_dir/host.err"
host_status=$?
set -e
...
echo "Distribution lifecycle reproducibility, install, update, rollback, and external consumer gates passed."
