...
temp_dir="$(mktemp -d /tmp/workspace-modular-distribution.XXXXXX)"
cleanup() {
  local original_status=$?
  case "$temp_dir" in
    /tmp/workspace-modular-distribution.*)
...
test ! -e "$journey_a/bin/gateway_sidecar"
jq -e '
  .schemaVersion == 2 and
...
"$journey_a/bin/workspace" --json distribution verify-bundle \
...
"$gateway/bin/workspace" --json distribution verify-bundle \
  --bundle "$gateway" >/dev/null

echo "Modular Distribution slim and reproducibility gates passed."
