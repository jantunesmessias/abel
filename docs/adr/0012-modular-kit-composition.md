# ADR-0012 — Composição modular configurável do DevExKit

Status: aceita em 2026-08-10.

## Contexto

O DevExKit implementou progressivamente catálogo, Studio, Sessions, App
...
- loaders diferentes voltam a ler `devex.local.yaml` para necessidades locais.
...
Consumer config v2 acrescenta `kit`, profiles, modules e provider bindings.
Config v1 permanece legível e é traduzida em memória para o profile
`legacy-full-local-v1`; v1 e v2 atravessam o mesmo resolver e composition root.
Não existe runtime legado paralelo.
...
2. Distribution v2 registra ModuleCatalog e permite bundles enxutos com
...
- Consumer config e Distribution ganham versões v2 incompatíveis no writer,
  mantendo readers v1.
...
- Resultados P0–V5 continuam evidência histórica e não são reescritos.

## Rollout e rollback

1. shadow resolution de config v1 sem mudança de comportamento;
2. composition root novo usando `legacy-full-local-v1`;
3. config v2 recomendada e migration preview-first;
4. Distribution v2 opt-in e depois default;
5. remoção do registro estático somente após paridade completa.

Rollback usa a distribuição imutável anterior e mantém a config v1 intacta.
Migration v2 possui dry-run, patch, ownership, backup e verificação antes de
apply.
...
- profiles `journey-preview`, `journey-android`, `gateway-lab`, `full-local` e
...
- gates históricos P0–V5 preservados.
