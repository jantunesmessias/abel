# ADR-0016 — Um único DevEx Studio em Jaspr
...
O baseline SR0–SR9 comprovou um Studio Flutter web conectado ao Host, com
...
`apps/devex_studio` será o único Studio e usará Jaspr em modo `client`. Não
...
```text
apps/devex_studio          Jaspr SPA, controllers e Host Client
packages/devex_ui_system  Jaspr + HTML/CSS, sem regra de domínio
packages/devex_ux_system  Dart puro, sem Jaspr ou Flutter
```

O Studio pode depender de `devex_contracts`, `devex_ui_system`,
`devex_ux_system`, `jaspr`, `jaspr_router`, `web` e transporte WebSocket web.
Ele não pode importar Flutter, Material, Cupertino, `dart:io`, `devex_engine`
ou `devex_runtime`.
...
`devex_ui_system` expõe componentes Jaspr e classes semânticas. CSS global é
...
somente no primeiro `devex.initialize`. URLs, HTML, build defines, logs,
...
`.dart_tool/devex/spikes/`. Antes do cutover, rollback significa descartar o
