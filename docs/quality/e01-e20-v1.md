# Matriz executável E-01…E-20 — V1 web/Android

Esta matriz liga cada critério normativo a uma evidência executável. O gate
composto é `tool/verify_v1_release.sh`; o vertical Android real é
...
| E-02 | `sidecar_stdio_test`, `devex_host_application_test` | processo, token, listener, runtime e traffic isolados por sessão |
...
| E-06 | `sidecar_stdio_test`, `verify_v02_containment.sh` | hybrid só delega decisão explícita; containment é reportado sem exagero |
| E-07 | `remote_config_provider_test` | provider genérico publica somente snapshot completo e válido |
| E-08 | `verify_v1_android.sh` | APK alcança Gateway por `adb reverse`, UI anuncia ready e cleanup zera recursos |
| E-09 | `gateway_runtime_test`, `gateway_contracts_test` | traffic é ordenado, limitado e sem headers sensíveis |
| E-10 | `session_capture_vault_test`, `app_adapter_capture_bridge_test`, `devex_flutter_binding_test` | TTL, principal, target, origin, capability one-shot e invalidação limitam captura temporária/direta |
...
| E-13 | parser/compiler e bundle V0 | content root contém documentos; binários entram apenas como artifact digest |
...
| E-16 | schemas, fixtures e gates V0.1–V1 | extensão atravessa catálogo, compile, handler, verify, docs, probe e testes |
| E-17 | `android_target_provider_test`, `verify_v1_android.sh` | dry-run/apply/verify/remove idempotentes, ownership e crash recovery |
