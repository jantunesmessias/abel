...
Status: normativo para hosted control plane/remote execution. Revisão: 2026-08-09. Owner: Security +
...
core local e a verificação offline de `.evidence.zip` não atravessam essas
fronteiras e continuam disponíveis quando hosted control plane/remote execution estão indisponíveis.
...
real do CNI, política de admissão que limite namespaces `workspace-run-*`,
...
3. processo da API → transação PostgreSQL com `control_plane.tenant_id` local;
...
| HR-03 | conexão pooled conserva tenant anterior | `SET LOCAL control_plane.tenant_id` dentro de transação; commit/rollback obrigatório; usuário da aplicação é `NOBYPASSRLS` | monitorar conexões sem transação e falhar startup se role puder bypassar RLS |
...
| HR-13 | RBAC permite workload fora do namespace/run | verbs mínimos sem list/watch/update; nomes de namespace são hash opaco; manifests têm labels e cleanup | RBAC não restringe nome em `create`; admission policy para `workspace-run-*` é gate obrigatório do cluster |
...
- `tools/hosted/verify_postgres_rls.sh` e
  `tools/hosted/verify_remote_scheduler_postgres.dart`;
- `tools/hosted/backup_postgres.sh`, `restore_rehearsal.sh` e
...
- `tools/verify/verify_kubernetes_manifests.sh` e `tools/verify/verify_supply_chain.sh`;
- ADR-0004, ADR-0005, resultados hosted control plane/remote execution e gates de `ARCHITECTURE.md`.
