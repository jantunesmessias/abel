...
`devex dev` resolve o `ResolvedKitPlan` e compila o catálogo antes de iniciar
...
Variant ou Evidence incrementa a revisão e publica `devex.workspace.changed`.
...
capturas independentes. O evento `devex.preview.changed` publica progresso; a
...
somente o cursor em memória e usa `devex.resume`. Cursor expirado retorna código
...
`workspace-snapshot` usa `application/json`. Artifact visual collected usa
`image/png` e purpose `visual-artifact`. O handle nunca contém path de CAS ou
workspace.

## Rotas do Studio

O servidor entrega `index.html` para GETs sem extensão que representam deep
links da SPA Jaspr (`/journeys/...`, `/target`, `/gateway`, `/reviews`,
`/remote/...` e `/hosted`). Um asset com extensão
ausente continua 404; não há fallback amplo. O Studio usa path URL strategy e
preserva a rota inicial de produção. Assets e bootstrap recebem `no-store`,
