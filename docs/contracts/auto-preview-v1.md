...
Status: contrato ativo AP1–AP4. Decisão: ADR-0013.
...
oficial e descobrível pelo DevEx Preview Compiler. A exportação de PNG não usa
uma API interna do Previewer: pertence ao runner isolado do DevExKit.

O package `devex_preview` depende apenas de Flutter e de contracts. Analyzer,
...
expande uma lista const de `AutoPreviewVariant`. A compatibilidade v1 é
`Flutter 3.44.x` e `widgetPreviewApi: experimental-v1`.
...
Todos os documentos conformam `schemas/v1/preview-capture.schema.json`, usam
...
`devex_preview`; semântica por nome textual isolado não basta. O compiler:
...
6. gera registry efêmero em `.dart_tool/devex/preview/<plan-digest>/`.
...
conformance do package `devex_preview`, do scanner e do spike no Previewer; não
