# ADR-0015 — UI System e UX System próprios do Abel

- Status: Aceita
- Data: 2026-08-10

Nota de evolução: a separação entre UI System e UX System continua aceita. A
implementação Flutter do `studio_ui` descrita neste ADR é substituída pela
...
design system externo. O Abel precisa suportar uma identidade própria,
...
- `interaction_model`: Dart puro; contém políticas de layout, interação, motion,
  disclosure, feedback e níveis de detalhe. Não depende de Flutter.
- `studio_ui`: depende de Flutter `widgets`/`rendering`, do UX System e de
...
Material/Cupertino também em `apps/studio/lib`; o gate executável em
`tools/gates/architecture_guard.dart` impede regressão. O histórico incremental explica
os commits intermediários, mas não autoriza adapters no runtime atual.

## Invariantes

1. Tokens do Studio são a única fonte de cor, tipografia, espaçamento, raio, stroke
...
6. O UI System não cria um catálogo de domínio paralelo: Scenario, Variant,
   Evidence e Module continuam vindo de `experience_contracts`.
...
O UI System expõe somente um subconjunto semântico por `StudioIcons`. A fonte é
