# ADR-0014 — Workspace autoritativo do Host para o Abel Studio

Status: aceita em 2026-08-10; reconstrução operacional do Studio concluída na matriz local.
...
pela ADR-0016; o baseline Flutter anterior permanece histórico nos resultados.

## Contexto

O Abel Studio atual possui uma UI mínima e usa um catálogo demonstrativo em
...
projeção de visual Evidence. `workspace dev` retorna instruções para iniciar Host e
Studio separadamente, em vez de supervisionar a experiência.

A composição modular e o AutoPreview entregaram fundamentos importantes, mas o
gate inicial do Studio comprovou apenas o seam de contributions/rotas, e o
AutoPreview, apenas a projeção tipada em memória. Esses gates não
comprovam a experiência operacional CAS → Host → Studio.

O Studio não pode corrigir esse gap lendo `workspace.yaml`, `.workspace`, `.dart_tool`
...
`workspace dev` resolve configuração e compila catálogo antes de efeitos, inicia o
...
`STUDIO_BOOTSTRAP_URL`; token e grants não entram no define ou na URL. Os
...
adiciona ao CI do Abel.

## Limitações normativas

- Widget Previewer continua experiência interativa, não exportador de PNG;
- o runner de auto-preview não usa API interna do Previewer;
- no baseline Flutter 3.47.0, spike do AutoPreview interativo está bloqueado por uma regressão do
  detector LSP/Pub Workspace, sem reclassificar o runner de auto-preview independente;
...
- `workspace dev` passa de planner para supervisor com cleanup obrigatório;
...
O rollout segue as capacidades operacionais do Studio em
`docs/architecture/studio-reconstruction-plan.md`. Cada gate é promovido
...
- `workspace dev --no-open` e shutdown sem orphan;
...
- auditoria requisito→mudança→teste→status antes da claim conformance e promoção do Studio.

O registro executado requisito por requisito está em
`docs/architecture/studio-reconstruction-results.md`. A decisão não
