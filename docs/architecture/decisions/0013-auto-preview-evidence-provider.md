...
O spike do AutoPreview no `examples/sample_flutter` comprovou que uma annotation
especializada pode:

- aparecer no Widget Previewer oficial;
- expandir duas Variants da mesma factory real;
- manter o entrypoint de produção sem import do Abel;
...
2. o Auto-preview compiler descobre os metadados const e produz um
   `PreviewManifest` canônico para um runner controlado.

O Abel não chama APIs internas do Previewer e não trata sua interface como
exportador de PNG. O runner de captura é uma implementação separada.

### 2. Boundary de package

`flutter_preview` isola a dependência experimental do Flutter. O package depende
somente de Flutter e `experience_contracts`; não depende de `dart:io`, engine ou
...
```text
.dart_tool/workspace/preview/<plan-digest>/
```
...
O rollout usa gates por capacidade: integração com o Widget Previewer,
API/contracts, scanner/compiler,
runner/Evidence e projeção no Journey Map. Até runner e Evidence do AutoPreview passar, nenhum resultado é
...
## Evolução do baseline em 2026-08-13

Flutter 3.47.0 alterou a API pública de temas do Previewer. O adapter passa a
declarar compatibilidade `3.47.x` e `widgetPreviewApi: experimental-v2`, sem
acoplar `flutter_preview` a Material. Os contratos do AutoPreview continuam cobertos pelo runner
auto-preview e passaram no novo baseline.

O fluxo interativo não é promovido: o Previewer 3.47.0 falha no gerador LSP na raiz do Pub
Workspace e não resolve o scaffold quando iniciado no package consumidor. A
opção legada usada no baseline 3.44.8 foi removida. A regressão interativa não
altera a Evidence estrutural nem autoriza uso de APIs internas do Flutter; o fluxo interativo
voltará ao gate apenas após correção upstream ou adapter dedicado verificável.
