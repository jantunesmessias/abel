# ADR-0009 — paridade web/Android: target, probes, bindings e retenção
...
web/Android precisa elevar o produto de preview local para uma matriz operacional
suportada web/Android, sem enfraquecer os limites estabelecidos em plataforma local–DISTRIBUTION LIFECYCLE.
...
criado pelo Abel; conhecer o serial ou ter bootstrap anterior não concede
...
- A claim web/Android é somente `web/Android`; Q-14/iOS permanece pós-remote execution.
- O gate Android precisa de Linux x86_64, KVM, imagem rootable pinada e ao menos
  8 GiB no parent efêmero do AVD.
- A distribuição pode avançar para `stable` somente após o gate composto web/Android.
- Capturas Android continuam `hostNative`; emulador nunca é `deviceAttested`.
- Evidência nativa além de PNG pertence a Android Evidence.
