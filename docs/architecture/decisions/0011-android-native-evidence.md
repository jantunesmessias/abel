# ADR-0011 — Android Evidence evidence nativa Android correlacionada

Status: aceita em 2026-08-09.

## Contexto

Android Evidence precisava capturar screenshot, semântica, logs, vídeo e performance no app
...
2. Emulador Android é sempre `hostNative`; Android Evidence não possui nem infere
...
5. O fingerprint inclui renderer, locale, timezone, ADB, build Android, Dart,
   ...
   `adb reverse`; Android Evidence não eleva o Android a `targetEnforced`.
...
- `tools/verify/verify_android_native_evidence.sh` e o modo `ANDROID_NATIVE_EVIDENCE=1` do gate Android;
...
- execução host-native registrada em `docs/architecture/android-native-evidence-results.md`.
