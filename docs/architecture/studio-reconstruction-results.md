# Resultados — único Abel Studio Jaspr

Data da evidência: 2026-08-11.

Este documento substitui a claim operacional anterior baseada no Studio Flutter.
A evidência Flutter de 2026-08-10 permanece baseline histórico no archive de
migração; não descreve o produto atual.

## Resultado arquitetural

- existe somente `apps/studio`;
- o app é Jaspr 0.23.3 em modo client-side;
- `studio_ui` usa Jaspr/HTML/CSS e Lucide;
- `interaction_model` é Dart puro;
...
- Flutter permanece em `flutter_preview`, `flutter_app_adapter` e consumers permitidos;
...
`.dart_tool/experience_platform/migration/flutter-studio-ui-baseline-20260810.tar.gz`, digest
...
- bootstrap exato `/studio/bootstrap.json`, Origin allowlisted e token efêmero;
...
- `dart analyze apps/studio libs/studio_ui
  libs/interaction_model libs/execution_runtime`: Pass;
...
`tools/probes/studio_jaspr_cdp_probe.dart` com Google Chrome passou após o cutover e
...
Studio. `verify_distribution_lifecycle.sh` também passou com dois bundles `full-local`
...
Atlas local. O Abel apresenta vantagem verificável para o domínio próprio:
