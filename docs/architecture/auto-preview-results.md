# Resultados — AutoPreview

Status: implementação e gates locais aprovados em 2026-08-10 para Flutter
3.44.8 / Dart 3.12.2.

Escopo: a projeção do AutoPreview no Journey Map comprova o contract/projector com handles fornecidos
por teste. A integração operacional CAS → Host → Studio, device frames,
inspector e coleta iniciada pelo Studio pertencem à reconstrução operacional do Studio e não são
claim deste resultado.

Atualização de proveniência: o vertical posterior do Studio implementou e
aprovou essa integração operacional. Isso não reescreve o gate do AutoPreview; a evidência
nova está em `studio-reconstruction-results.md`.

## Resultado entregue

`flutter_preview` oferece `AutoPreview`, `AutoMultiPreview` e
...
## Gates do AutoPreview

| Gate | Resultado observado |
|------|---------------------|
| spike do AutoPreview | Widget Previewer oficial encontrou as annotations e Variants com `--legacy-preview-detection` |
| contratos do AutoPreview | API Flutter, `Variant`, descriptors/manifests/report e schema v1 conformes |
| scanner e compiler do AutoPreview | scanner fail-closed e registry efêmero; sem build_runner ou geração em `lib/` |
| runner e Evidence do AutoPreview | subprocesso por descriptor, PNGs reais, falha parcial, timeout, quotas, CAS e Evidence |
| projeção do AutoPreview no Journey Map | projeção Journey Map in-memory com handle injetado, fidelidade e status/freshness explícitos |
...
O gate focado passou no vertical AutoPreview:
