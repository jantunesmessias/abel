# Resultado executado da plataforma local
...
|---|---|---|
| contratos headless | implementado | schemas/fixtures, compiler deterministico, CAS/locks e CLI validate/explain/compile |
| compreensão estática | implementado | Studio, deep links, estados imutaveis, a11y widgets, Journey Map benchmark e auditoria Orca/Chromium |
| execução web local | implementado | Host RPC real, origins separados, lifecycle/cancel/reset e 20 ciclos sem residuo |
| evidence e release | implementado | PNG validado, pixel digest, freshness, CAS, PublicationView, bundle offline e CLI capture/release build |

O caminho direto de captura tambem foi fechado: `capture.request` emite
...
`docs/protocols/host-app-adapter.md`.

## Consumer real e fluxo de evidence/release
...
O gate compila `examples/sample_flutter/tool/target_main.dart`, serve o build em
...
estao em `docs/quality/studio-accessibility.md`.
...
- O resultado deste arquivo nao faz claim de Gateway; o vertical posterior
  GATEWAY ISOLATED esta registrado separadamente em `docs/architecture/gateway-isolation-results.md`.
  Host-native, Android e iOS continuam fora desta evidencia plataforma local.
