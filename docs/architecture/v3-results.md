...
O modo V3 de `tool/verify_v1_android.sh` terminou com exit `0` no run
...
O `.devexbundle` de 584.667 bytes foi verificado offline com archive digest
...
Semântica e logcat exportados foram validados como `hashedTextV1` e
`hashedMessageV1`, sem campos crus. Mídia foi autorizada porque o sample usa
...
V3 não afirma iOS, dispositivo físico, hosted collaboration, remote runtime ou
device farm. Esses itens permanecem V4/V5 ou posteriores.
