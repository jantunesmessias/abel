# Resultado V5 — remote runtime web/Android
...
```bash
dart test packages/devex_contracts/test/remote_execution_contracts_test.dart
dart test packages/devex_runtime/test/remote_scheduler_service_test.dart \
  packages/devex_runtime/test/remote_kubernetes_dispatcher_test.dart \
  packages/devex_runtime/test/remote_worker_service_test.dart \
  packages/devex_runtime/test/system_remote_worker_backend_test.dart \
  packages/devex_runtime/test/scrcpy_remote_session_test.dart \
  packages/devex_runtime/test/kubernetes_remote_job_test.dart \
  packages/devex_runtime/test/remote_cleanup_reconciler_test.dart
...
CHROME_EXECUTABLE=/usr/bin/chromium \
  flutter test --platform chrome test/remote_session_surface_web_test.dart
```
...
```bash
dart run tool/hosted/verify_remote_scheduler_postgres.dart
```
...
3. aplicar admission policy que restrinja o ClusterRole a namespaces
   `devex-run-*` com labels imutáveis;
...
renderizado ou mocks e estão ligados ao threat model V4/V5.
