# Resultado executado da paridade web/Android
...
```text
workspace target android discover
workspace target android start|managed-status|stop
workspace target android bootstrap|update|remove|verify
workspace target android install|launch|reset|capture
workspace target android tls-install|tls-verify|tls-remove
```
...
O sample Android usa package `io.github.jantunesmessias.sample_flutter`. Somente o
entrypoint de tooling importa `flutter_app_adapter`; o plugin nativo lê extras
`uppercase launch keys` limitados e rejeita nomes secret-like. O build de produção continua
na factory neutra do consumidor.

Execução real de `tools/verify/verify_android_evidence.sh`:
...
- Activity exata resumed e `Semantics.identifier=gateway.health`;
...
reproduzível está em `tools/probes/studio_accessibility_driver.dart`; ele complementa,
mas não substitui, o teste manual com leitor de tela.

## Gate e limites

A correspondência E-01…E-20 está em `docs/quality/platform-evidence-matrix.md`. O gate local
composto também reconstrói distribuição `0.1.0`, executa conformance externa e
compara alias/canônico.

Execução integral final de `tools/verify/verify_release.sh`:
...
web/Android afirma somente web/Android. Não afirma iOS, dispositivo físico, assinatura
de package/OCI, evidence Android ampliada, hosted ou device farm. Q-13, Q-15 e
Q-16 estão fechadas; Q-14 continua formalmente pós-remote execution.
