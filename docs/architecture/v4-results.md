# Resultado V4 — SaaS multi-tenant e colaboração otimista
...
- control plane Dart como modular monolith sobre `devex_engine`;
...
O core local e `.devexbundle` não dependem do control plane hosted.
...
```bash
dart test packages/devex_contracts/test/hosted_contracts_test.dart
dart test packages/devex_runtime/test/hosted_collaboration_service_test.dart \
  packages/devex_runtime/test/oidc_pkce_authenticator_test.dart \
  packages/devex_runtime/test/s3_object_store_test.dart
...
PostgreSQL 18.4 local, com role `devex_app NOBYPASSRLS`, recebeu as seis
migrations. Os gates executados foram:

```bash
./tool/hosted/verify_postgres_rls.sh
...
| core local e bundle offline sem hosted | Preservado pelos gates V0–V3/CLI |
