...
Future<void> main() async {
  final adminUrl = _requiredEnvironment('DEVEX_DATABASE_URL');
  final applicationUrl = _requiredEnvironment('DEVEX_APP_DATABASE_URL');
  ...
  try {
    await admin.run((session) async {
      ...
      for (final tenant in <String>[tenantA, tenantB]) {
        await session.execute(
          Sql.named('''
            INSERT INTO devex_hosted.principals
        ...
        await session.execute(
          Sql.named('''
            INSERT INTO devex_hosted.workspaces
    ...
    try {
      await admin.run((session) async {
        await session.execute(
          Sql.named('''
              DELETE FROM devex_hosted.remote_runs
        ...
        await session.execute(
          Sql.named('''
              DELETE FROM devex_hosted.workspaces
              WHERE tenant_id = ANY(@tenants:_text)
            '''),
          parameters: <String, Object?>{
            'tenants': <String>[tenantA, tenantB],
        ...
        await session.execute(
          Sql.named('''
            DELETE FROM devex_hosted.organizations
            WHERE tenant_id = ANY(@tenants:_text)
          '''),
          parameters: <String, Object?>{
            'tenants': <String>[tenantA, tenantB],
