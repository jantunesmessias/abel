...
Future<void> main(List<String> arguments) async {
  ...
  final temporary =
      arguments.isEmpty
            ? Directory.systemTemp.createTempSync('devex-external-consumer-')
            : Directory(arguments.single)
        ..createSync(recursive: true);
  final ownsTemporary = arguments.isEmpty;
  try {
    final copied = Directory(p.join(temporary.path, 'packages'))..createSync();
    ...
    for (final name in packageNames) {
      _copyPackage(
        Directory(p.join(repository, 'packages', name)),
...
dependencies:
  flutter:
    sdk: flutter
  devex_contracts:
    path: ../packages/devex_contracts
  devex_engine:
    path: ../packages/devex_engine
  devex_runtime:
    path: ../packages/devex_runtime
  devex_flutter:
    path: ../packages/devex_flutter
  devex_testkit:
    path: ../packages/devex_testkit
dev_dependencies:
  flutter_test:
    sdk: flutter
''');
...
import 'package:devex_external_consumer/public_api.dart';
