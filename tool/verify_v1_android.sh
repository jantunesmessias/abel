...
host_port="${DEVEX_GATEWAY_HOST_PORT:-45901}"
target_port="${DEVEX_GATEWAY_TARGET_PORT:-55901}"
emulator_port="${DEVEX_ANDROID_EMULATOR_PORT:-5556}"
system_image="${DEVEX_ANDROID_SYSTEM_IMAGE:-system-images;android-35;google_apis;x86_64}"
avd_parent="${DEVEX_ANDROID_AVD_PARENT:-/dev/shm}"
...
if ss -Hln "sport = :$host_port" | rg -q . || ss -Hln "sport = :$target_port" | rg -q .; then
  echo "Configured V1 Gateway ports are already in use." >&2
  exit 3
fi
...
avd_root="$(mktemp -d "$avd_parent/devex-v1-avd.XXXXXX")"
avd_name="DevExKit_V1_$$"
gateway_pid=""
cleanup_verified=0
evidence_dir="$repo_dir/.dart_tool/devex/v1-gate/run-$$"
mkdir -p "$evidence_dir"

devex() {
  ANDROID_AVD_HOME="$avd_root" dart run apps/devex_cli/bin/devex.dart \
    --json target android "$@" --sdk-root "$sdk_root"
}

devex_root() {
  ANDROID_AVD_HOME="$avd_root" dart run apps/devex_cli/bin/devex.dart --json "$@"
}

cleanup() {
  local original_status=$?
  set +e
  devex tls-remove --apply >/dev/null 2>&1
  devex remove --apply >/dev/null 2>&1
  if devex stop --apply >/dev/null 2>&1; then
    cleanup_verified=1
  fi
  ...
  if [[ "$cleanup_verified" -eq 1 || ! -e .dart_tool/devex/devex-kit/android/managed-emulator-v1.json ]]; then
    case "$avd_root" in
      "$avd_parent"/devex-v1-avd.*) find "$avd_root" -depth -delete 2>/dev/null ;;
...
devex start --avd "$avd_name" --port "$emulator_port" --headless --apply \
  | tee "$evidence_dir/android-start.json"
devex bootstrap --serial "emulator-$emulator_port" --host-port "$host_port" \
  --target-port "$target_port" --apply \
  | tee "$evidence_dir/android-bootstrap.json"
devex verify | tee "$evidence_dir/android-verify.json"
devex install --serial "emulator-$emulator_port" --apk "$apk" \
  | tee "$evidence_dir/android-install.json"
devex launch --serial "emulator-$emulator_port" \
  --package dev.devexkit.sample_flutter --activity .MainActivity \
  --host-port "$host_port" --target-port "$target_port" \
  --overlay DEVEX_SESSION_ID=v1-gate-session \
  --overlay DEVEX_SESSION_NONCE=0123456789abcdef \
  | tee "$evidence_dir/android-launch.json"

for _ in $(seq 1 30); do
  "$sdk_root/platform-tools/adb" -s "emulator-$emulator_port" \
    shell uiautomator dump /sdcard/devex-v1-window.xml >/dev/null 2>&1 || true
  if "$sdk_root/platform-tools/adb" -s "emulator-$emulator_port" \
    shell cat /sdcard/devex-v1-window.xml 2>/dev/null \
    | rg -q 'resource-id="devex.gateway.health"[^>]+content-desc="Gateway: ready:android-ok"'; then
    break
  fi
  sleep 1
done
"$sdk_root/platform-tools/adb" -s "emulator-$emulator_port" \
  shell cat /sdcard/devex-v1-window.xml \
  | rg -q 'resource-id="devex.gateway.health"[^>]+content-desc="Gateway: ready:android-ok"'

devex capture --serial "emulator-$emulator_port" \
  | tee "$evidence_dir/android-capture.json"
rg -q '"artifactDigest":"sha256:[0-9a-f]{64}"' "$evidence_dir/android-capture.json"

if [[ "${DEVEX_V3_EVIDENCE:-0}" == "1" ]]; then
  devex_root compile --config examples/sample_flutter/devex.yaml \
  ...
  devex evidence --serial "emulator-$emulator_port" \
    ...
    --package dev.devexkit.sample_flutter --launch-profile android \
  ...
  (cd examples/sample_flutter && dart run ../../apps/devex_cli/bin/devex.dart --json \
  ...
  devex_root evidence compare-visual \
  ...
  devex_root evidence compare-semantics \
  ...
  devex_root release build --config examples/sample_flutter/devex.yaml \
    | tee "$evidence_dir/android-v3-release.json"
  release_directory="$(jq -r '.result.directory' "$evidence_dir/android-v3-release.json")"
  devex_root release bundle --directory "$release_directory" \
    --output "$evidence_dir/android-v3.devexbundle" \
    | tee "$evidence_dir/android-v3-bundle.json"
  devex_root release verify-bundle --bundle "$evidence_dir/android-v3.devexbundle" \
    | tee "$evidence_dir/android-v3-bundle-verify.json"
fi

devex reset --serial "emulator-$emulator_port" \
  --package dev.devexkit.sample_flutter \
  | tee "$evidence_dir/android-reset.json"
if "$sdk_root/platform-tools/adb" -s "emulator-$emulator_port" \
  shell pidof dev.devexkit.sample_flutter >/dev/null 2>&1; then
  echo "Application process remained after package reset." >&2
  exit 5
fi

devex tls-install --apply | tee "$evidence_dir/android-tls-install.json"
devex tls-verify | tee "$evidence_dir/android-tls-verify.json"
remote_path="$(jq -r '.remotePath' .dart_tool/devex/devex-kit/android/tls-v1.json)"
"$sdk_root/platform-tools/adb" -s "emulator-$emulator_port" shell test -f "$remote_path"
devex tls-remove --apply | tee "$evidence_dir/android-tls-remove.json"
test ! -e .dart_tool/devex/devex-kit/android/tls-v1.json
test ! -e .dart_tool/devex/devex-kit/android/tls
...
devex remove --apply | tee "$evidence_dir/android-remove.json"
devex stop --apply | tee "$evidence_dir/android-stop.json"
cleanup_verified=1
...
echo "V1 Android lifecycle, Gateway health, capture, reset, TLS, and undo passed."
