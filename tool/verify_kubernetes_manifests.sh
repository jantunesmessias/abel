...
validation_root="$(mktemp -d /tmp/devex-kubernetes-validation.XXXXXX)"

cleanup() {
  if [[ "${DEVEX_KEEP_VALIDATION_ARTIFACTS:-false}" == "true" ]]; then
...
if [[ -n "${DEVEX_HELM_BIN:-}" ]]; then
  helm_bin="$DEVEX_HELM_BIN"
else
...
if [[ -n "${DEVEX_KUBECONFORM_BIN:-}" ]]; then
  kubeconform_bin="$DEVEX_KUBECONFORM_BIN"
else
...
web_image="registry.example.test/devex/worker-web@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
android_image="registry.example.test/devex/worker-android@sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
...
"$helm_bin" lint deploy/helm/devex-hosted \
  --set-string "image.digest=$hosted_digest"
"$helm_bin" lint deploy/helm/devex-hosted \
  --set-string "image.digest=$hosted_digest" \
  "${remote_values[@]}"
"$helm_bin" template devex-default deploy/helm/devex-hosted \
  --set-string "image.digest=$hosted_digest" \
  --output-dir "$validation_root/chart-default"
"$helm_bin" template devex-remote deploy/helm/devex-hosted \
  --set-string "image.digest=$hosted_digest" \
  "${remote_values[@]}" \
  --output-dir "$validation_root/chart-remote"

dart run tool/render_remote_jobs.dart "$validation_root/remote-jobs"
...
for route in "${route_manifests[@]}"; do
  jq -e '
    ...
    .kind == "HTTPRoute" and
    (.metadata.namespace | startswith("devex-run-")) and
