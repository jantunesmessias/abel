...
final class DistributionDescriptor {
  DistributionDescriptor({
    ...
    _catalogMaxLength(displayName, 'displayName', 512);
...
final class ConsumerLayout {
  ConsumerLayout({
    ...
    this.configEnvironmentVariable = 'WORKSPACE_CONFIG',
    List<String> commandAliases = const <String>['workspace'],
  }) : commandAliases = List<String>.unmodifiable(commandAliases) {
  ...
  static final ConsumerLayout standard = ConsumerLayout(
    configFile: 'workspace.yaml',
    contentRoot: '.experience',
    localConfigFile: 'workspace.local.yaml',
...
List<T> _catalogSorted<T>(
  ...
  if (result.length > maxItems || _catalogDuplicates(result.map(key))) {
    throw ArgumentError(
      '$path IDs must be unique and contain at most $maxItems items',
    );
  }
...
void _catalogMaxLength(String value, String name, int maxLength) {
  if (value.length > maxLength) {
...
final class Workspace {
  Workspace({required this.id, required this.displayName}) {
    _catalogNonEmpty(displayName, 'displayName');
    _catalogMaxLength(displayName, 'displayName', 512);
  }
...
final class Application {
  Application({
    ...
    _catalogNonEmpty(displayName, 'displayName');
    _catalogMaxLength(displayName, 'displayName', 512);
    _catalogNonEmpty(root, 'root');
    _catalogMaxLength(root, 'root', 4096);
    _catalogNonEmpty(target, 'target');
    _catalogMaxLength(target, 'target', 256);
  }
...
final class Journey {
  Journey({
    ...
    _catalogMaxLength(title, 'title', 2048);
    if (this.scenarioIds.length > 100000 ||
        _catalogDuplicates(this.scenarioIds.map((id) => id.value))) {
      throw ArgumentError(
        'scenarioIds must be unique and contain at most 100000 items',
      );
    }
  }
...
final class Scenario {
  Scenario({
    ...
    _catalogMaxLength(title, 'title', 2048);
    if (description != null) {
      _catalogNonEmpty(description!, 'description');
      _catalogMaxLength(description!, 'description', 16384);
    }
    if (this.sourceReferences.length > 1000) {
      throw ArgumentError('sourceReferences must contain at most 1000 items');
    }
  }
...
final class Transition {
  Transition({
    ...
    if (label != null) {
      _catalogNonEmpty(label!, 'label');
      _catalogMaxLength(label!, 'label', 2048);
    }
  }
...
final class SourceReference {
  SourceReference({required this.repository, required this.path, this.symbol}) {
    _catalogNonEmpty(repository, 'repository');
    _catalogMaxLength(repository, 'repository', 512);
    _catalogNonEmpty(path, 'path');
    _catalogMaxLength(path, 'path', 4096);
    if (symbol != null) {
      _catalogNonEmpty(symbol!, 'symbol');
      _catalogMaxLength(symbol!, 'symbol', 1024);
    }
  }
...
final class ScenarioExecutionBinding {
  ScenarioExecutionBinding({
    ...
    _catalogMaxLength(targetId, 'targetId', 256);
    ...
    if (launchProfileId != null) {
      ...
      _catalogMaxLength(launchProfileId!, 'launchProfileId', 256);
    }
    if (checkpointId != null) {
      OpaqueId.validate(checkpointId!, 'Checkpoint');
      _catalogMaxLength(checkpointId!, 'checkpointId', 256);
    }
    if (gatewayPresetId != null) {
      OpaqueId.validate(gatewayPresetId!, 'GatewayPreset');
      _catalogMaxLength(gatewayPresetId!, 'gatewayPresetId', 256);
    }
  }
...
final class ReviewGuideStep {
  ReviewGuideStep({
    ...
    _catalogMaxLength(instruction, 'instruction', 4096);
    _catalogMaxLength(observationCriteria, 'observationCriteria', 4096);
...
final class ReviewGuide {
  ReviewGuide({
    ...
    if (steps.isEmpty ||
      ...
      throw ArgumentError(
        'ReviewGuide steps must be non-empty, unique and bounded',
      );
    }
  }
...
void _validateCatalogManifest(CatalogManifest manifest) {
  ...
  final journeyById = <JourneyId, Journey>{
    for (final journey in manifest.journeys) journey.id: journey,
  };
  ...
  for (final journey in manifest.journeys) {
    if (!applicationIds.contains(journey.applicationId) ||
        journey.scenarioIds.any((id) {
          final scenario = scenarioById[id];
  ...
  for (final transition in manifest.transitions) {
    ...
    if (!journeyIds.contains(transition.journeyId) ||
      ...
      throw ArgumentError(
        'Transition ${transition.id} has an unknown or cross-Journey reference',
      );
    }
  }
