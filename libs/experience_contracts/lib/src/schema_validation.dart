...
final class Draft202012Validator {
  ...
  static const Set<String> _namedSchemaMapKeywords = <String>{
    r'$defs',
    'definitions',
    'dependentRequired',
    'dependentSchemas',
    ...
    if (value case final Map<Object?, Object?> map) {
      for (final entry in map.entries) {
        ...
        if (_unsupportedProfileKeywords.contains(key)) {
          throw FormatException('$key is outside the supported schema profile');
        }
        if (key == r'$ref' &&
            entry.value is String &&
            !(entry.value! as String).startsWith('#')) {
          throw const FormatException(
            'External references are outside the supported schema profile',
          );
        }
        if (key == 'enum' &&
        ...
        if (_namedSchemaMapKeywords.contains(key) &&
            nested is Map<Object?, Object?>) {
          for (final namedEntry in nested.entries) {
            if (namedEntry.key is! String) {
              throw FormatException(
                'Schema key must be a string at $path.$key',
              );
            }
            _validateProfile(namedEntry.value, '$path.$key.${namedEntry.key}');
          }
        } else {
