...
_ConformanceResult _verifyJsonSchema(Directory repository) {
  ...
  for (final file in files) {
    ...
    for (final rawGroup in groups) {
      ...
      if (!_isSupportedSchemaProfile(group['schema'])) {
...
bool _isSupportedSchemaProfile(Object? value) {
  if (value case final Map<String, Object?> map) {
    for (final entry in map.entries) {
      ...
      if (!_isSupportedSchemaProfile(entry.value)) return false;
    }
  } else if (value case final List<Object?> list) {
    return list.every(_isSupportedSchemaProfile);
  }
  return true;
}
