...
void main() {
  ...
  test('source documents are strict, canonical, and schema conformant', () {
    ...
    final validator = Draft202012Validator(
      jsonDecode(
            File(
              p.join(_root(), 'schemas/source/source-automation.schema.json'),
            ).readAsStringSync(),
          )
          as Object,
    );
  ...
  test(
    ...
    () {
      final entry = EvidenceBundleEntry(
        path: 'release.json',
      ...
      final manifest = EvidenceBundleManifest(
        ...
        entries: <EvidenceBundleEntry>[entry],
      );
      ...
      final bundleValidator = Draft202012Validator(
        jsonDecode(
              File(
                p.join(_root(), 'schemas/evidence/evidence-bundle.schema.json'),
              ).readAsStringSync(),
            )
            as Object,
      );
      final pluginValidator = Draft202012Validator(
        jsonDecode(
              File(
                p.join(_root(), 'schemas/source/plugin-manifest.schema.json'),
              ).readAsStringSync(),
            )
            as Object,
      );
      ...
      expect(
        EvidenceBundleManifest.fromJson(manifest.toJson()).digest,
        manifest.digest,
      );
