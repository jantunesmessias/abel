...
void main() {
  late Directory temp;
  setUp(
    () => temp = Directory.systemTemp.createTempSync('evidence-bundle-test.'),
  );
  tearDown(() {
  ...
  test('builds byte-identical ZIPs and verifies the release offline', () {
    ...
    const repository = DeterministicEvidenceBundleRepository();
    final first = repository.build(
      releaseDirectory: releaseDirectory.path,
      outputPath: p.join(temp.path, 'one.evidence.zip'),
    );
    final second = repository.build(
      releaseDirectory: releaseDirectory.path,
      outputPath: p.join(temp.path, 'two.evidence.zip'),
    );
    expect(
  ...
  test('rejects trailing data and path traversal before extraction', () {
    ...
    final built = repository.build(
      releaseDirectory: releaseDirectory.path,
      outputPath: p.join(temp.path, 'valid.evidence.zip'),
    );
    final trailing = File(p.join(temp.path, 'trailing.evidence.zip'))
    ...
    final traversal = File(p.join(temp.path, 'traversal.evidence.zip'))
