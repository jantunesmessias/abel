...
void main() {
  ...
  setUp(() {
    temporary = Directory.systemTemp.createTempSync('workspace-supervisor-');
  });
  ...
  test(
    ...
    () async {
      ...
      expect(
        await _status(
          runtime.hostOrigin.replace(path: '/studio/bootstrap.json'),
        ),
        HttpStatus.notFound,
      );
      expect(
      ...
      expect(utf8.decode(deepLink.bytes), contains('Abel Studio'));
      ...
      final deniedBootstrap = await _get(
        runtime.studioOrigin!.replace(path: '/studio/bootstrap.json'),
      );
