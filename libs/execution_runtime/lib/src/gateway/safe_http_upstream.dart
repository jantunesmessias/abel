...
final class GatewayUpstreamEndpoint {
  GatewayUpstreamEndpoint({
    ...
    final environmentHandle =
        handle != null &&
        RegExp(r'^env:[A-Z][A-Z0-9_]{0,63}$').hasMatch(handle);
    final capturedSessionHandle = handle == 'session:authorization';
    if (handle != null && !environmentHandle && !capturedSessionHandle) {
      throw ArgumentError(
        'credentialHandle must name an uppercase environment variable or '
...
final class EnvironmentCredentialResolver implements CredentialResolver {
  ...
  Future<CredentialValue?> resolve(String handle) async {
    if (!RegExp(r'^env:[A-Z][A-Z0-9_]{0,63}$').hasMatch(handle)) {
