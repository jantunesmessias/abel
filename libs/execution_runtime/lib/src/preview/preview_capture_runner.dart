...
final class LocalFlutterTestProcessExecutor implements PreviewProcessExecutor {
  ...
  Future<PreviewProcessResult> execute({
    ...
    environment['PREVIEW_ROBOTO_FONT'] = fonts.roboto;
    environment['PREVIEW_MATERIAL_ICONS_FONT'] = fonts.materialIcons;
...
final class PreviewScaffoldBuilder {
  ...
  String _source(
    ...
    final buffer = StringBuffer()
      ...
      ..writeln('    await _loadPreviewFonts();')
      ..writeln(
    ...
    for (final descriptor in descriptors) {
      ...
      buffer
        ...
        ..writeln(
          '    // PREVIEW_OUTPUT ${jsonEncode(outputPaths[descriptor.key])}',
        )
        ..writeln(
          "    print('PREVIEW_${protocolToken}_PNG "
          "${base64Encode(utf8.encode(descriptor.key))} "
          "\${base64Encode(data.buffer.asUint8List())}');",
        )
        ..writeln('    } on Object catch (error, stackTrace) {')
        ..writeln(
          "      print('PREVIEW_${protocolToken}_ERROR "
          "${base64Encode(utf8.encode(descriptor.key))} "
    ...
    buffer
      ...
      ..writeln('}')
      ..writeln()
      ..writeln('Future<void> _loadPreviewFonts() async {')
      ..writeln("  await _loadPreviewFont('Roboto', 'PREVIEW_ROBOTO_FONT');")
      ..writeln(
        "  await _loadPreviewFont('MaterialIcons', "
        "'PREVIEW_MATERIAL_ICONS_FONT');",
      )
      ..writeln('}')
      ..writeln()
      ..writeln(
        'Future<void> _loadPreviewFont(String family, String environmentKey) async {',
      )
...
final class PreviewCaptureRunner {
      ...
      try {
        ...
        final process = await processExecutor.execute(
          ...
          completionMarkers: <String>{
            'PREVIEW_${scaffold.protocolToken}_PNG ',
            'PREVIEW_${scaffold.protocolToken}_ERROR ',
          },
  ...
  void _materializeProtocolOutput(
    ...
    final pngPrefix = 'PREVIEW_${scaffold.protocolToken}_PNG ';
    final errorPrefix = 'PREVIEW_${scaffold.protocolToken}_ERROR ';
    final seen = <String>{};
