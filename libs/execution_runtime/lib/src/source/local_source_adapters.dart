...
final class LocalContextBundleExporter {
  ...
  (String, bool) _redact(String source) {
    ...
    final patterns = <RegExp>[
      ...
      RegExp(
        r'''["']?(authorization|cookie|set-cookie|token|secret|password|api[_-]?key|private[_-]?key|connection[_-]?string|dsn)["']?\s*[:=]\s*(?:["'][^"'\r\n]*["']|[^\s,}\r\n]+)''',
        caseSensitive: false,
      ),
      RegExp(r'\bBearer\s+[A-Za-z0-9._~+/%-]+={0,2}', caseSensitive: false),
      RegExp(
        r'''\b(?:postgres(?:ql)?|mysql|mariadb|mongodb(?:\+srv)?|redis|rediss|amqp|amqps)://[^\s"'<>]+''',
        caseSensitive: false,
      ),
      RegExp(
